# levenshtein_codeopt.py
# Usage: python levenshtein_codeopt.py /tmp/out.txt
...
def lev_rowrolling(a: str, b: str) -> int:
    ...
    for i, ca in enumerate(a, 1):
        cur = [i] + [0]*nb
        prev_j1 = prev[1:]          # cache slices
        prev_j0 = prev[:-1]
        bj = b                       # bind str
        cj = cur
        for j, cb in enumerate(bj, 1):
            ins = prev_j1[j-1] + 1
            del_ = cj[j-1] + 1
            sub = prev_j0[j-1] + (ca != cb)
            cj[j] = ins if ins < del_ else (del_ if del_ < sub else sub)
        prev = cur
    return prev[-1]
...
if __name__ == "__main__":
    ...
    _ = lev_rowrolling(s1, s2)  # warmup
    t0 = time.perf_counter()
    for _ in range(RUNS):
        _last = lev_rowrolling(s1, s2)
