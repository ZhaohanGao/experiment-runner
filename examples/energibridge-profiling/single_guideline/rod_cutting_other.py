# floyd_warshall_other.py
# Usage: python floyd_warshall_other.py /tmp/out.txt
...
def floyd_warshall_numpy(dp):
    n = dp.shape[0]
    for k in range(n):
        # bulk update with broadcasting
...
if __name__ == "__main__":
    ...
    if np is None:
        print("NumPy not available."); open(out,"w").write("0.0"); sys.exit(0)

    base_dp = make_random_graph_np(n, density, w_min, w_max, seed)
    t0 = time.perf_counter()
    for _ in range(runs):
        dp = base_dp.copy()
        floyd_warshall_numpy(dp)
