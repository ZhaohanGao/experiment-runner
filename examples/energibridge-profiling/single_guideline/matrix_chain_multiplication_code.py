...
def matrix_chain_multiply(arr: List[int]) -> int:
    if len(arr) < 2:
        return 0
    n = len(arr)
    dp = [[maxsize]*n for _ in range(n)]  # G7: list mult for row
    # G6: set diagonal once
    for i in range(1, n):
        dp[i][i] = 0
    # G3: store ranges once
    for i in range(n-1, 0, -1):
        ai_1 = arr[i-1]          # G1: cache hot reads
        for j in range(i+1, n):
            aj = arr[j]          # G1
            best = dp[i][j]
            row_i = dp[i]; row_ip1 = dp  # local binds no big benefit but consistent
            for k in range(i, j):
                cost = dp[i][k] + dp[k+1][j] + ai_1 * arr[k] * aj
                if cost < best:
                    best = cost
            dp[i][j] = best
    return dp[1][n-1]
...
if __name__ == "__main__":
    ...
    out = sys.argv[1]
    N = int(os.getenv("MCM_N", "800"))
    GEN = os.getenv("MCM_GEN", "range").lower()
