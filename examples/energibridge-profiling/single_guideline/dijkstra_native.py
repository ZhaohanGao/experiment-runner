...
# G15: The performance-critical `dijkstra` function is targeted for compilation.
# G16: We use the `@njit` decorator, which is equivalent to `@jit(nopython=True)`.
# This is a compiler flag that generates highly optimized native code without
# falling back to the Python interpreter, ensuring maximum performance.
@njit
def dijkstra_numba(graph: list, start_node: int, end_node: int, num_nodes: int) -> int:
    """
    Return the cost of the shortest path between integer-indexed nodes.
    This version is optimized for Numba's JIT compilation.
    """
    # Numba works efficiently with simple data types like NumPy arrays.
    # A boolean array is used instead of a set for tracking visited nodes.
    visited = np.zeros(num_nodes, dtype=np.bool_)
    
    # The heap stores tuples of (cost, node_index).
    heap = [(0, start_node)]
    ...
    while heap:
        ...
        # Accessing neighbors from the list-based graph structure is very fast.
        for v, c in graph[u]:
            if not visited[v]:
                next_cost = cost + c
...
def prepare_graph_for_numba(graph_dict: dict):
    """
    Converts the string-based dictionary graph into a Numba-friendly format
    (list of lists with integer indices).
    """
    # Map city names to unique integer indices
    node_to_idx = {name: i for i, name in enumerate(graph_dict)}
    
    num_nodes = len(node_to_idx)
    numba_graph = List([List.empty_list(types.UniTuple(types.int64, 2)) for _ in range(num_nodes)])

    # Build the integer-indexed adjacency list
    for u_name, neighbors in graph_dict.items():
        u_idx = node_to_idx[u_name]
        for v_name, cost in neighbors:
            v_idx = node_to_idx[v_name]
            numba_graph[u_idx].append((v_idx, cost))
            
    return numba_graph, node_to_idx, num_nodes
...
if __name__ == "__main__":
    ...
    # G17: Avoid heavy calls in loops. We perform the heavy data structure
    # conversion once, before the timed benchmark loop begins.
    numba_graph, node_to_idx, num_nodes = prepare_graph_for_numba(COMPLEX_GRAPH)

    # Warm-up run: Numba compiles the function on its first call.
    # Running it once ensures the compilation time isn't part of our measurement.
    for start_node_name, end_node_name in test_cases:
        start_idx = node_to_idx[start_node_name]
        end_idx = node_to_idx[end_node_name]
        dijkstra_numba(numba_graph, start_idx, end_idx, num_nodes)
    ...
    for _ in range(num_executions):
        for start_node_name, end_node_name in test_cases:
            # The lookup from name to index is a fast O(1) dictionary operation.
            start_idx = node_to_idx[start_node_name]
            end_idx = node_to_idx[end_node_name]
            # Call the highly optimized, pre-compiled native code function.
            dijkstra_numba(numba_graph, start_idx, end_idx, num_nodes)
