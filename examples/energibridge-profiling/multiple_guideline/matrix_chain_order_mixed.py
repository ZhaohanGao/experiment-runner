...
if NUMBA_OK:
    ...
    def _mco_flat_numba(arr_np: np.ndarray):
        n = arr_np.size
        INF = 2**62
        dp  = np.full(n*n, 0, dtype=np.int64)
        sol = np.zeros(n*n, dtype=np.int64)

        def idx(i,j): return i*n + j

        # init off-diagonals to INF
        for i in range(1, n):
        ...
        for chain_len in range(2, n):
            for a in range(1, n - chain_len + 1):
                ...
                base_a = a*n
                for c in range(a, b):
                    cost = dp[base_a + c] + dp[(c+1)*n + b] + ai_1 * arr_np[c] * ab
                    if cost < best:
                        best = cost; sol[idx(a,b)] = c
                dp[idx(a,b)] = best

        # reshape to 2D for return
        dp2  = dp.reshape((n,n))
        sol2 = sol.reshape((n,n))
        return dp2, sol2
...
def matrix_chain_order(array: List[int]) -> Tuple[List[List[int]], List[List[int]]]:
    ...
    if n==2:
        z=[[0]*n for _ in range(n)]; return z,[row[:] for row in z]
    if NUMBA_OK:
        m, s = _mco_flat_numba(np.asarray(array, dtype=np.int64))
        return m.tolist(), s.tolist()

    # Fallback: flat Python
    INF = sys.maxsize
    nn = n*n
    dp  = [0]*nn
    sol = [0]*nn
    ...
    for chain_len in range(2,n):
        for a in range(1,n-chain_len+1):
            ...
            for c in range(a,b):
                ...
                if cost<best: best=cost; sol[idx(a,b)]=c
            dp[idx(a,b)] = best
...
if __name__=="__main__":
    ...
    _ = matrix_chain_order(dims)  # warmup / JIT
